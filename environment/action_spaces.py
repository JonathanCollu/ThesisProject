...
class SelectMove(object):
  ...
  def step(self, action, sprites, keep_in_frame):
    ...
    clicked_sprite, index = self.get_sprite_from_position(position, sprites)
    if clicked_sprite is not None:
      others = [sprites[i] for i in range(len(sprites)) if i != index]
      clicked_sprite.move(motion, keep_in_frame=keep_in_frame, sprites=others)
